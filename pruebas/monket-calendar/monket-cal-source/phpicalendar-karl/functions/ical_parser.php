...
function isWebCal($filename) {
	return substr($filename, 0, 7) == 'http://' 
		|| substr($filename, 0, 8) == 'https://'
                || substr($filename, 0, 9) == 'webcal://';
}
...
if ($save_parsed_cals == 'yes') {	
	$parsedcal = $calendar_path . $cal_filename . '.cached';
	if (file_exists($parsedcal)) {
		$parsedcal_mtime = filemtime($parsedcal);

		if ($cal == $ALL_CALENDARS_COMBINED) {
			// get latest mtime for each calendar
			
			$latestModifiedTime = 0;
			foreach ($cal_filelist as $filename) {
				$modifiedTime = 0;
				if (isWebCal($filename)) {
					if (time() - $parsedcal_mtime > 60 * 60) {
						$modifiedTime = $parsedcal_mtime + 1;
					}
				} else {
			...
			$readCache = $latestModifiedTime < $parsedcal_mtime;		
		} else if ($is_webcal) {
			if (time() - $parsedcal_mtime > 60 * 60) {
				$readcache = false;
			} else {
			...
			$readCache = $realcal_mtime < $parsedcal_mtime;
		}
...
foreach ($cal_filelist as $filename) {
	...
	$actual_calname = basename($filename, '.ics');
...
//			 exit(error($error_invalidcal_lang, $filename));
		...
		$master_array['calendars'][$calnumber]['id'] = $actual_calname;
		$master_array['calendars'][$calnumber]['name'] = $actual_calname;
		$master_array['calendars'][$calnumber]['filename'] = $filename;
		$master_array['calendars'][$calnumber]['isWebCal'] = isWebCal($filename);
				...
				if ((isset($allday_start) && $allday_start != '')) {
					...
					if ($start < $mArray_end) {
						while (($start != $end) && ($start < $mArray_end)) {
							...
							$master_array[($start_date2)][('-1')][$uid]= array ('allday_start' => $allday_start, 'allday_end' => $allday_end , 'event_text' => $summary, 'description' => $description, 'location' => $location, 'organizer' => serialize($organizer), 'attendee' => serialize($attendee), 'calnumber' => $calnumber, 'calname' => $actual_calname, 'url' => $url, 'status' => $status, 'class' => $class );
			...
			// Begin VTODO Support
				...
				switch ($property) {
					...
					case 'UID':
						...
						break;
					case 'X-WR-CALNAME':
						$master_array['calendars'][$calnumber]['name'] = $data;
						$actual_calname = $data;
